...
gpx_original = gpd.GeoDataFrame.from_features([feature for feature in gpx_original], crs=gpx_original.crs)
# Copy to a version to be changed
gpx = gpx_original

# getting bound and expanding to the plot
aoi_bounds = gpx.geometry.total_bounds
xlim = ([aoi_bounds[0]-.05,  aoi_bounds[2]+.05])
ylim = ([aoi_bounds[1]-.05,  aoi_bounds[3]+.05])
...
met = pd.read_csv(WEATHER_FORECAST)  # source https://meteostat.net/en/station/87178?t=2023-04-04/2023-04-10
# filtering columns
...
#                    color_discrete_sequence=px.colors.sequential.Plasma_r,
#                    template="plotly_dark",)
fig = px.scatter_polar(traj.df, r=traj.df.index, theta=traj.df.wdir, #color="strength",
                    color=traj.df.wspd,
                    template="plotly_dark",)
fig.show()
...
ax.set_xlim(xlim)
ax.set_ylim(ylim)
traj.plot("speed", linewidth = 3, legend=True,  legend_kwds={'shrink': 0.3}, ax=ax, cmap="Reds")
